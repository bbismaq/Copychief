...
**Ordem de leitura dentro de cada nicho (corrigida pelo usuário, 2026-07-15): fonte 1 (doc curado) primeiro, fonte 2 (pasta de transcrições) depois.** Havia uma contradição interna neste SKILL.md — este parágrafo dizia "fonte 2 primeiro" enquanto a lista numerada logo abaixo já tinha fonte 1 como item 1 e fonte 2 como item 2; a lista estava certa, o parágrafo estava desatualizado. **Isso NÃO muda o peso de decisão:** fonte 1 continua só confirmando/priorizando entre os ângulos que aparecerem recorrentes na mineração exaustiva da fonte 2 — nunca decide a big idea sozinha (ver regra abaixo), mesmo sendo lida primeiro agora. Isso vale independente dos links fixos ou da renumeração de 2026-07-14 — só a ORDEM de leitura mudou, não qual fonte manda na decisão final.
...
2. **Criativos externos escalados transcritos** (fonte 2, fonte-base de repertório) — pasta da tabela acima, uma por nicho. São **dezenas** de ads que JÁ ESCALARAM, em texto (EN + PT-BR).
   - **Sempre partir do LINK da pasta (tabela acima), nunca de uma busca às cegas por nome (regra do usuário, 2026-07-15 — "não tem código de nicho, eu já te mandei o link da pasta").** O "código do nicho" abaixo é só um padrão observado nos nomes dos arquivos, não uma convenção que substitui o link — a pasta que o usuário linkou é a fonte de verdade, e o método abaixo é só a forma de driblar as limitações técnicas das ferramentas pra enxergar o conteúdo completo DAQUELA pasta específica.
   - **Como enumerar TODOS os arquivos da pasta (descoberto em 2026-07-10, refinado em 2026-07-15):** `listFolderContents` com `folderId` da tabela acima **e `maxResults: 100` explícito** (o default é 50) já mostra os docs `TRANSCRIÇÃO -` misturados com os `.mp4`, em ordem alfabética — rodar essa chamada PRIMEIRO, sempre, pra confirmar visualmente que a pasta é a certa e ter uma amostra real do conteúdo. Se a pasta tiver mais de 100 itens (vídeos + docs somados), essa chamada sozinha ainda corta antes do fim — não tem paginação exposta nessa ferramenta. Nesse caso, complementar com `searchDriveFiles` **sem `folderId`** (esse parâmetro está QUEBRADO nessa ferramenta — testado de novo em 2026-07-15, continua retornando `"Failed to search files: Invalid Value"`), buscando pelo **código do nicho embutido no nome do arquivo** (`WL` para Weight Loss, `ED` para ED, `MMR` para Memory Loss — `ML` NÃO funciona como código pra Memory Loss, é ruído: pega docs de microlead/outros produtos que só coincidem na substring; testado e descartado em 2026-07-15), `mimeType: document`, confirmando `hasMore: false`. **Depois de rodar a busca por código, cruzar pelo menos 2-3 IDs de arquivo do resultado contra o que apareceu na chamada `listFolderContents` da pasta linkada — só assim fica confirmado que a busca por nome não trouxe arquivo de fora da pasta certa nem perdeu nada por causa do código errado.** Filtrar do resultado os docs que não são transcript (relatórios, briefings antigos, atas de reunião) — ficam só os `TRANSCRIÇÃO - ...`. Risco conhecido: um ad salvo sem o código do nicho no nome fica invisível pra essa busca — se a contagem final parecer baixa demais pro que se espera ("dezenas"), vale checar a pasta visualmente antes de fechar a mineração como completa.
...
- **O princípio "copiar, nunca construir do zero" vale pra QUALQUER bloco de conteúdo já formatado dentro do doc clonado, não só pro clone inicial do arquivo inteiro (regra do usuário, 2026-07-15 — causa raiz de uma cascata de erros de formatação na seção EN de uma demanda).** Se o doc clonado já traz uma seção pronta com a formatação certa (ex.: a seção `TRADUÇÃO (EN)` que já vem do próprio doc-modelo, com `spaceAbove`/`spaceBelow`/`TITLE` corretos), **editar o TEXTO dentro dela** — trocar código do hook, hook, corpo, frase por frase, via `deleteRange`+`insertText` mirando só o trecho que muda — **nunca apagar o bloco inteiro pra digitar de novo puro**. A formatação de parágrafo vive na marca de parágrafo, não no texto corrido; apagar o parágrafo junto com o texto descarta a formatação inteira e obriga reconstruir tudo na mão depois (negrito, espaçamento, estilo TITLE) — foi exatamente aí que os erros se acumularam.
...
- **Armadilhas técnicas (não repetir erros já cometidos):**
  ...
  - **`cloneTable` + `deleteTableRows` (técnica pra adicionar uma linha extra numa tabela, ex.: ir de 4 pra 5 hooks) NÃO herda 100% do estilo da linha original — conferir SEMPRE com um diff completo antes de dar como pronto (regra do usuário, 2026-07-15).** A linha "extra" pode sair com `paragraphStyle.namedStyleType` diferente do esperado (ex.: `NORMAL_TEXT` em vez do `HEADING_2` que a célula de código do hook normalmente usa — é esse heading que dá negrito por padrão, não só o `textStyle` explícito) e sem o `tableCellStyle` (padding 5pt em cada lado, `contentAlignment: "TOP"`) que as outras células da tabela têm. Depois de clonar+cortar, comparar explicitamente `paragraphStyle`, `textStyle` E `tableCellStyle` da linha nova contra uma linha normal da mesma tabela via `readDocument(format:"json")` — não só o texto ou um único atributo isolado (ex.: só o tamanho da fonte, achando que resolveu).
  - **Verificação de formatação não pode confiar na mensagem de sucesso da ferramenta (`applyTextStyle`/`applyParagraphStyle` dizendo "aplicado no range X-Y") — reler o JSON fresco depois e conferir o range REAL do textRun resultante (regra do usuário, 2026-07-15).** Em mais de uma ocasião o range reportado pela ferramenta não bateu com o range realmente aplicado no documento (ex.: tentar corrigir negrito de `"Big Idea:"` cortou o próprio rótulo ao meio, porque o range usado no fix estava desalinhado do real). Sempre reconferir run por run com leitura fresca depois de qualquer correção — "a ferramenta disse sucesso" não é prova de que o resultado está certo.
  - **Antes de montar qualquer seção baseada num modelo (ex.: seção EN/tradução), ler o `readDocument(format:"json")` do documento de referência e extrair as propriedades REAIS de parágrafo (`spaceAbove`, `spaceBelow`, `namedStyleType`) — não só copiar o padrão de texto (regra do usuário, 2026-07-15).** Construir a seção via `insertText` de um bloco de texto puro (sem essas propriedades) sai sem o respiro visual entre linhas que o modelo tem, mesmo que o CONTEÚDO bata perfeitamente palavra por palavra. "O texto tá igual" não é prova de que a formatação bate.
...
**Lição crítica sobre como manter esse gate (2026-07-21):** o erro do check H (parágrafo em branco faltando no corpo) não era desconhecido — a regra já estava escrita na seção "Corpo (PT)" acima ("cada parágrafo separado por linha em branco") ANTES dessa demanda. O checklist simplesmente nunca tinha sido auditado contra as regras já documentadas em outras partes desta skill, só contra bugs que já tinham vazado uma vez. **Por isso: sempre que uma regra de formatação for escrita em qualquer parte desta skill (bloco-título, hooks, corpo, seção EN, nomenclatura), checar na hora se o `validar_criativos` já tem um check automatizado equivalente — se não tiver, adicionar um check novo ali mesmo, não esperar a regra ser violada numa demanda real pra descobrir o gap.** O checklist abaixo precisa cobrir 100% das regras documentadas nesta skill, não só o histórico de bugs já vistos.

**Protocolo:**

1. **Ler o doc em JSON via `readDocument` format=json** (arquivo salvo em chunks se truncar).
2. **Rodar os 9 checks abaixo** (via script Node no arquivo salvo, ou subagent — reportar resultado de cada um, não só "passou"):
   ...
   - **G. Seção EN:** parágrafo `TRADUÇÃO (EN)` usa `namedStyleType: "TITLE"`; todos os demais parágrafos da seção têm `spaceAbove`/`spaceBelow` batendo com o padrão do modelo (12pt/12pt no Memopezil atual); nenhuma palavra do mecanismo/nicho antigo do template (usar os termos do PRÓPRIO modelo de origem daquela demanda, não uma lista fixa — ex.: `"rosemary"`/`"Dr. Oz"` se veio do Primal Brain, `"honey"`/`"Okinawa"` se veio de outro) sobrou.
   - **H. Toda célula CORPO com mais de uma frase tem parágrafo em branco entre cada frase** (não só quebra de linha simples). Detectar: dentro da célula de corpo, contar parágrafos com texto vs. parágrafos vazios — pra N frases tem que haver exatamente N-1 parágrafos vazios intercalados. **Armadilha que causou esse check (2026-07-21, demanda Memomatrix):** `replaceTableRowData` recebe as frases como uma única string; se as frases forem juntadas com `\n` simples, o tool cria um parágrafo por frase mas **sem** o parágrafo em branco entre elas — quebra visualmente o espaçamento que o resto do doc usa (título, hooks e o próprio corpo do modelo original usam parágrafo em branco intercalado, confirmado inspecionando o JSON de um doc real aprovado). **Fix:** juntar as frases com `\n\n` (duas quebras), não `\n` — isso faz o `replaceTableRowData` criar o parágrafo vazio real entre cada frase. Testar sempre com uma dump do JSON puro da célula (não confiar no campo `text` de `getTableStructure`, que concatena tudo com `\n` simples e esconde os parágrafos em branco reais). **A seção EN faz o oposto do corpo em tabela — corpo em EN NÃO leva parágrafo em branco entre frases** (segue o modelo, ver seção "Seção EN" acima); parágrafos em branco na seção EN só devem existir como separador ENTRE criativos, nunca dentro do corpo de um único criativo.
   - **I. Nenhum hook termina com o marcador residual `"usado"`/`"used"` colado ao texto** (defeito recorrente herdado de docs antigos copiados como referência — ver regra "Hooks" acima). Detectar: `textRun` da coluna de texto do hook terminando em `usado`/`used` (com ou sem pontuação antes) sem que isso faça parte natural da frase.
...
- **Se uma auto-revisão (não pedida) encontrar algo que pareça errado mas que o usuário não apontou, APRESENTAR o achado e esperar confirmação antes de alterar — nunca aplicar a mudança primeiro (regra do usuário, 2026-07-15, repetição de uma regra já quebrada antes nessa mesma conta).** Vale mesmo quando a mudança parece óbvia/correta pelo próprio julgamento — "eu vi que tava diferente do modelo de referência" não é autorização pra editar sem avisar.
