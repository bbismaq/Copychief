...
1. **Criativos externos escalados transcritos** (fonte-base de repertório) — pasta `Swipe de ADS Validados > Ads externos escalados > <nicho>` (Weight Loss / Memory Loss / ED). São **dezenas** de ads que JÁ ESCALARAM, em texto (EN + PT-BR). Já estão transcritos localmente também (scratchpad `all_transcripts` = WL, `memo_transcripts` = ML, `ed_transcripts` = ED) — dá pra minerar de lá, mais barato.
   ...
   - **Prova obrigatória de mineração (regra do usuário, 2026-07-10):** ao apresentar as 6 big ideas (passo 3), incluir sempre uma seção `Mineração desta demanda` listando: quantos ads foram abertos **de fato nesta conversa** (não citação de mineração anterior) + 3-5 nomes/IDs de doc como amostra verificável. Se essa seção não existir na apresentação, a mineração não aconteceu — é o próprio usuário quem vai checar isso agora, então não faz sentido tentar pular.
